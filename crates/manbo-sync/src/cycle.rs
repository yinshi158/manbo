...
use crate::merge::{WholeFileOutcome, merge, merge_whole_file};
use crate::registry::{MergePolicy, SyncFile, SyncScope};
use crate::state::{SyncReport, SyncState, base_path};
...
fn sync_one(
    ...
    if file.policy == MergePolicy::WholeFile {
        ...
        return sync_whole_file(backend, data_dir, file, state, sides);
    }
...
/// 整文件合并的三侧内容与远端版本号（参数多，打个包传）。
struct WholeFileSides<'a> {
    /// 上次合并达成一致的内容。
    base: Option<&'a str>,
...
/// 远端一次取回的三件：内容、版本号、mtime（毫秒）。
type Fetched = (Option<String>, Option<String>, Option<u128>);

/// 撞车后重取远端；拿不到内容（刚被删了）就按没有远端重并。
fn refetch(backend: &mut dyn SyncBackend, name: &str) -> Result<Fetched, SyncError> {
