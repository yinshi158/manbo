...
mod tests {
    ...
    fn requires_a_url() {
        ...
        assert!(matches!(
            WebdavBackend::new(&config),
            Err(SyncError::MissingUrl)
        ));
    }
    ...
    fn splits_the_scheme_host_for_mkcol() {
        ...
        assert_eq!(
            backend.file_url("user.tsv"),
            "https://dav.jianguoyun.com/dav/manbo/user.tsv"
        );
    ...
    fn tag_falls_back_to_length_and_last_modified() {
        ...
        headers.insert(
            ...
            HeaderValue::from_static("Mon, 14 Sep 2026 00:00:00 GMT"),
        );
        assert_eq!(
            tag_from_headers(&headers, 123),
            "123-Mon, 14 Sep 2026 00:00:00 GMT"
        );
