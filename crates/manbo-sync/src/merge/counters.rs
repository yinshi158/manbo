...
pub(super) fn merge(key_columns: KeyColumns, cap: Option<u64>, base: &str, local: &str, remote: &str) -> String {
    ...
    for key in keys {
        ...
        for column in 0..width {
            let count = |rows: &Rows| -> i64 {
                rows.get(key).and_then(|values| values.get(column)).copied().unwrap_or(0) as i64
            };
    ...
    for line in data_lines(text) {
        ...
        let (key, counts) = match key_columns {
            ...
            KeyColumns::AllButLast => (
                fields[..fields.len() - 1].iter().map(|field| (*field).to_owned()).collect(),
                fields[fields.len() - 1].trim().parse::<u64>().map(|count| vec![count]),
            ),
        };
...
mod tests {
    ...
    fn sums_independent_deltas_from_both_sides() {
        // 基准 10；本机 +5，另一台 +8 → 23
        let merged = dispatch(MergePolicy::Counters, Some("微博\t10\n"), "微博\t15\n", "微博\t18\n");
        assert_eq!(merged, "微博\t23\n");
    }

    #[test]
    fn first_sync_without_base_adds_both_sides() {
        let merged = crate::merge::merge(MergePolicy::Counters, None, Some("吧\t3\n"), Some("把\t4\n"));
    ...
    fn repeated_merges_are_idempotent() {
        ...
        // 合并后三侧一致：再并一遍不能变
        let again = crate::merge::merge(MergePolicy::Counters, Some(&once), Some(&once), Some(&once));
    ...
    fn deletion_propagates_as_a_negative_delta() {
        ...
        let merged = crate::merge::merge(MergePolicy::Counters, Some("微博\t10\n"), Some(""), Some("微博\t10\n"));
        assert_eq!(merged, "");
    ...
    fn union_max_caps_at_the_fixed_frequency() {
        ...
        let merged = crate::merge::merge(MergePolicy::UnionMax, Some("曼波\t100\n"), Some(""), Some("曼波\t100\n"));
        assert_eq!(merged, "");
    }
}
