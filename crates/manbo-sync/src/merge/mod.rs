...
pub fn merge(
    ...
    match policy {
        MergePolicy::Counters => {
            counters::merge(counters::KeyColumns::AllButLast, None, base, local, remote)
        }
        MergePolicy::DailyStats => {
            counters::merge(counters::KeyColumns::First, None, base, local, remote)
        }
