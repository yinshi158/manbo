...
- `CloudGlossFiller`：释义兜底（Core `GlossFiller` trait，与 Predictor 分开的线程与通道，攒 1.5 秒 / 8 个词发一次，问过不再问）：
  随包释义表没有的词库词 / 云端词上屏后入队，结果壳每秒 `Engine::poll_glosses` 经 `Translator::learn` 写进 `manbo-translate::PersonalGlossary`
...
学习数据同步（设计见 `docs/design/sync.md`）。核心是三方计数合并 `merged = remote + (local − base)`，
`base` 快照在 `<数据目录>/sync/base/<文件>`，状态（设备 id、每文件版本号、上次成功 / 失败）在
`sync/state.json`（设置页直接读它显示）。模块：`merge`（五种策略：Counters 键 = 除末列外全部 /
DailyStats 键 = 首列多计数列 / UnionMax 词频 100 封顶 / Vocabulary 日期 min-max / Glossary 键值并集本地优先，
整文件三方 `WholeFile` 走 `merge_whole_file`，双方都改取 mtime 新的、输方落 `config.conflict-*.toml`）/
`registry`（`resolve(数据目录, scope)` 列名单：`Learning` = 六表 + config.toml，`Full` 加 usage / vocab / 释义表）/
`backend`（`SyncBackend` trait + `webdav`（GET/PUT/MKCOL + ETag 乐观并发，412 由 cycle 重取重并 ≤3 次，无 PROPFIND；
`ensure_root` 逐段 MKCOL，**建成 / 405 / 403 都算通过**——坚果云对固定根路径 `/dav` 的 MKCOL 恒回 403，
只放行 405 会让每轮同步在第一段就放弃、远端永远空着，见 `docs/design/sync.md`「后端」）
与 `folder`（目录当云，版本号 = mtime 毫秒-字节数，兼作测试与 CLI 后端））/ `cycle`（一轮编排：取远端 → 合并 →
原子写本地 → 上传 → 重读校验后记基准）/ `service`（`SyncService` 常驻线程 + `run_once` 限时一次性，壳的
启动 / 退出用后者）。壳接线：启动 `run_once(Full)`（限时，失败不挡启动）；秒级 / 消息节拍上到点
`flush_learning` → `request(Learning)`，报告到了且空闲 `Engine::set_learner(FrequencyLearner::from_path(…))`
热替换（**set_learner 不 flush 旧的**，磁盘已是合并结果）；macOS 停用输入源、Windows `serve_pipe` 退出各再推一轮；
Windows 设置页「同步」与 macOS 菜单「立即同步」触发。CLI：`manbo-cli --sync <云目录> --data-dir <设备目录>`
离线验证。永不同步：`input-log.jsonl`、`.env`、`dicts/`、`model/`、`logs/`、`sync/` 自身。
失败退避：自动间隔 ×2 封顶 ×8，成功归一。

## crates/manbo-format

`.qj` 数据容器（`Container` mmap 读、`Writer` 写、`Table<T>` / `Text` 零拷贝视图、`hash` 可落盘哈希索引、`Metadata` 名称 / 许可证 / 署名）。
词库与语言模型都能 `write_qj` / 从 `.qj` 打开，启动 50 ms；`cargo run --release -p manbo-dict-convert -- pack dict|lm --name … --license …`
...
`cargo run --release -p manbo-dict-convert -- bigram --phrases assets/lexicon/phrases.tsv --brand assets/lexicon/brand.tsv data/corpus/*.txt` 生成；语料在 `data/corpus/`（gitignore）。
短语层不当 token 统计（分词时摘掉、统计完按成分合成一元 / 二元，短语得分等于原来两个词的路径，见 `bigram.rs` 模块注释），品牌词按给定次数写进一元与句首二元。

## crates/manbo-platform
...
自己解析 `trak` 字距表、按主题 gamma 加深笔画；cosmic-text 打了 `opsz` 光学字号补丁（manbo-team/cosmic-text 分支 `manbo-opsz`，workspace `[patch.crates-io]` 钉 rev）。
...
测试工具，`cargo run -p manbo-cli -- kaifa`。
...
- `apps/macos/scripts/bundle.sh --install` 打包安装到 `~/Library/Input Methods/`（开发用），`--pkg` 做分发用的 pkg（装 `/Library/Input Methods/`，postinstall 跑 `manbo-macos --register`
  注册、启用并切成当前输入源；签名 / 公证靠 `MANBO_SIGN_IDENTITY` / `MANBO_INSTALLER_IDENTITY` / `MANBO_NOTARY_PROFILE`，没设就 ad-hoc；`MANBO_TARGET` 指定架构，
  成品 `target/pkg/Manbo-<版本>-<arm64|x86_64>.pkg`）；`scripts/uninstall.sh` 卸载。
- 日志在 `~/Library/Logs/Manbo/`（按天分文件留 7 天，删了会重建），用户数据与配置在 `~/Library/Application Support/Manbo/`。
- 配置项：云联想 `[predict]`（偏好设置「云服务」页有「测试连接」按钮：`manbo_predict::ConnectionTest` 起线程发一条最小请求，`Host` 用独立定时器 `CloudTestMonitor` 轮询结果显示到窗口底部；
...
- 输入法进程由 launchd 拉起，看不到 shell 的环境变量：密钥写进配置同目录的 `.env`（`MANBO_API_KEY=...`，输入法启动时 dotenvy 读入）或 `config.toml` 的 `api_key`。
- 本地整句模型：`bundle.sh` 把 `data/model/`（或 `MANBO_MODEL_DIR`）三件套打进 `Resources/model/`，用户目录 `model/` 优先；`host/model.rs` 在后台线程加载并预热（首次 Metal 编译）后
...
一个产品两个 package：`server`（Server 进程：IPC 分派 + Engine + 命名管道 + 自绘候选窗与悬浮状态条）与 `tsf`（TSF 文本服务 DLL，lib 名固定 `manbo_tsf`），
外加 `settings`（WinUI 3 设置程序）与 `installer`（Inno Setup）。不合成一个 crate，因为 DLL 不能带 Engine 的依赖树，见 `apps/windows/README.md`；
协议类型在 `manbo-platform::protocol`，设计见 `docs/design/architecture.md`「Windows：TSF」。
...
- `assets/emoji/emoji-zh.tsv` / `emoji-en.tsv`：Unicode CLDR 中文 / 英文 annotations 转出的 emoji 表（Unicode License v3，可发布；中文词与英文词各配 emoji，两张表加载时合成一张），
  `cargo run --release -p manbo-dict-convert -- --out-dir assets/emoji emoji --language zh data/cldr/annotations-zh.json data/cldr/annotationsDerived-zh.json`（en 同理）。
...
用 LLM 批量生成释义表：`cargo run --release -p manbo-gloss-gen -- generate`（密钥读 `MANBO_API_KEY`，结果 JSONL 在 `data/generated/`，不进 git、可续跑，`--limit 80` 试跑）
...
- `lexicon`：从 `assets/lexicon/`（自建词库源：规范字 + 常用词 + THUOCL 领域词）加 Unihan 读音（`data/unihan/Unihan_Readings.txt`）、LLM 多音字标注（`gloss-gen pinyin`，
  ...
  `dicts/<领域>.tsv` + `.qj`（11 本、13 万条，`--domain-keep-min`），流程见 `assets/lexicon/MANBO.md`；`--extra-words` 并入人工挑的领域词 `assets/lexicon/domain_words.tsv`。
- `english`：转 `assets/lexicon/05_english/00_all_words.tsv`；`cedict`：释义表备用来源。
- `bigram`：统计语料；`--phrases` 给短语层、`--brand` 给品牌词（`assets/lexicon/brand.tsv`，曼波 210），领域词也走合成计数（语料里只有几十次的词当 token 统计会吸走成分词的二元证据）。
