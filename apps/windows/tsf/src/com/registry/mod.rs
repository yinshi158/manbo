...
use crate::com::{
    CLSID_MANBO, CLSID_MANBO_STR, GUID_PROFILE, LANGID_ZH_CN, SERVICE_DESCRIPTION,
};
...
fn unregister_profile() -> Result<()> {
    com_scope(|| {
        if let Ok(category) = unsafe {
            ...
            for catid in CATEGORIES {
                let _ =
                    unsafe { category.UnregisterCategory(&CLSID_MANBO, catid, &CLSID_MANBO) };
            }
        }
        if let Ok(profiles) = unsafe {
            ...
            unsafe {
                let _ =
                    profiles.RemoveLanguageProfile(&CLSID_MANBO, LANGID_ZH_CN, &GUID_PROFILE);
